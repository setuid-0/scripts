...
# Script para estimar el tiempo de desarrollo de un proyecto Ansible y asignar un nivel de complejidad
...
function print_title() {
    local title="$1"
    echo -e "\e[1;31m===== $title =====\e[0m"  # Color rojo
}
...
inventory_val=$(( times["inventory_val_base"] + (num_hosts * 1) + (num_groups * 2) ))
...
# Tabla de criterios:
# | Complejidad | Sistemas | Reglas de negocio/Políticas | Integraciones | Actividades | Tiempo (horas) |
# |-------------|----------|----------------------------|---------------|-------------|----------------|
# | Alta plus   |     4    |             7              |       3       |   Hasta 8   |       8        |
# | Media plus  |     2    |             5              |       2       |   Hasta 5   |       5        |
# | Baja plus   |     1    |             2              |       1       |   Hasta 3   |       3        |
# | Alta        |     4    |             7              |       0       |   Hasta 8   |       8        |
# | Media       |     2    |             4              |       0       |   Hasta 5   |       5        |
# | Baja        |     1    |             2              |       0       |   Hasta 3   |       3        |

# Contadores para categorías
count_sistemas=0
count_reglas=0
count_integraciones=0
count_actividades=0

# Sistemas
if [ "$custom_project_structure" -eq 1 ]; then
    count_sistemas=$((count_sistemas + 1))
fi
if [ "$ansible_cfg_dev" -gt 0 ]; then
    count_sistemas=$((count_sistemas + 1))
fi
if [ "$inventory_dev" -gt 0 ]; then
    count_sistemas=$((count_sistemas + 1))
fi
if [ "$use_ssh_config" -eq 1 ]; then
    count_sistemas=$((count_sistemas + 1))
...
if [ "$variables_dev" -gt 0 ]; then
    count_reglas=$((count_reglas + 1))
fi
if [ "$total_vars_role_dev" -gt 0 ]; then
    count_reglas=$((count_reglas + 1))
fi
if [ "$use_tags" -eq 1 ]; then
    count_reglas=$((count_reglas + 1))
...
# Integraciones
if [ "$total_module_dev" -gt 0 ]; then
    count_integraciones=$((count_integraciones + num_modules))
fi
if [ "$use_collections" -eq 1 ]; then
    count_integraciones=$((count_integraciones + 1))
fi

# Actividades
if [ "$total_playbook_dev" -gt 0 ]; then
    count_actividades=$((count_actividades + num_playbooks))
fi
if [ "$total_role_dev" -gt 0 ]; then
    count_actividades=$((count_actividades + num_roles))
fi
if [ "$total_task_dev" -gt 0 ]; then
    count_actividades=$((count_actividades + (total_tasks / num_roles)))
fi
if [ "$total_template_dev" -gt 0 ]; then
    count_actividades=$((count_actividades + num_templates))
fi
if [ "$total_handler_dev" -gt 0 ]; then
    count_actividades=$((count_actividades + num_handlers))
fi
if [ "$total_static_file_dev" -gt 0 ]; then
    count_actividades=$((count_actividades + num_static_files))
fi

# Determinación del nivel de complejidad
complexity=""

if [ "$count_sistemas" -ge 4 ] && [ "$count_reglas" -ge 7 ] && [ "$count_integraciones" -ge 3 ]; then
    complexity="Alta Plus"
elif [ "$count_sistemas" -ge 2 ] && [ "$count_reglas" -ge 5 ] && [ "$count_integraciones" -ge 2 ]; then
    complexity="Media Plus"
elif [ "$count_sistemas" -ge 1 ] && [ "$count_reglas" -ge 2 ] && [ "$count_integraciones" -ge 1 ]; then
    complexity="Baja Plus"
elif [ "$count_sistemas" -ge 4 ] && [ "$count_reglas" -ge 7 ] && [ "$count_integraciones" -eq 0 ]; then
    complexity="Alta"
elif [ "$count_sistemas" -ge 2 ] && [ "$count_reglas" -ge 4 ] && [ "$count_integraciones" -eq 0 ]; then
    complexity="Media"
elif [ "$count_sistemas" -ge 1 ] && [ "$count_reglas" -ge 2 ] && [ "$count_integraciones" -eq 0 ]; then
    complexity="Baja"
else
...
echo -e "\e[1;33mNivel de Complejidad:\e[0m $complexity"
echo -e "\e[1;32m==========================================================\e[0m"
