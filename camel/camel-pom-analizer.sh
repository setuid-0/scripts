...
is_valid_pom() {
  ...
  # Verificar si el archivo contiene al menos los elementos mínimos para considerarse un POM válido
  if grep -q "<artifactId>" "$POM_FILE" && grep -q "<version>" "$POM_FILE"; then
    return 0
  fi
